...
const { getAssetById } = require('./federation');
...
const getPlanets = async (federation_account, mining_account, eos_rpc) => {
  ...
  for (let p = 0; p < planets_res.rows.length; p++) {
    ...
    // trilium reserve
    const planet_reserve = await eos_rpc.get_currency_balance(
      'alien.worlds',
      pr.planet_name,
      'TLM'
    );
    ...
    pr.total_stake = (
      pr.total_stake / Math.pow(10, parseInt(dac_precision))
    ).toFixed(dac_precision);
...
const getBag = async (mining_account, account, eos_rpc, api_endpoints) => {
  ...
  if (bag_res.rows.length) {
    ...
    bag = await Promise.all(
      bag_res.rows[0].items.map(async (x) => {
        const a = await api_endpoints.getAsset(account, x);
        const data = await a.data();
        return { data };
      })
    );
    // bag = api_endpoints.getAsset(account, bag_res.rows[0].items);
  }
  return bag;
};
...
const getLand = async (
  ...
  const asset = await api_endpoints.getAsset(land_owner, land_id);
...
const doWork = async ({
  ...
  const rand_str = Array.from(rand_arr)
...
const doWorkWorker = async (mining_params) => {
  ...
  const _doWorkWorker = async (_message) => {
    ...
    console.log(
      `Found hash in ${itr} iterations with ${account} ${rand_str}, last = ${last}, hex_digest ${hex_digest} taking ${
        (end - start) / 1000
      }s`
    );
...
const getBountyFromTx = async (transaction_id, miner, api_endpoints) => {
  // temporary fix
  await sleep(4000);

  return new Promise(async (resolve, reject) => {
    for (let i = 0; i < 10; i++) {
      for (let a = 0; a < api_endpoints.length; a++) {
        const api_endpoint = api_endpoints[a];
        let url = `${api_endpoint}/v1/alienworlds/mines?tx_id=${transaction_id}`;
        const t_res = await fetch(url);
        const t_json = await t_res.json();
        // console.log(t_json);
        if (
          t_json.results &&
          t_json.results.length &&
          t_json.results[0].bounty
        ) {
          resolve(`${t_json.results[0].bounty / 10000} TLM`);
          return;
        }
        await sleep(1000);
      }
...
const processRandomQueue = async (
  mining_account,
  eos_api,
  permission = 'active'
) => {
