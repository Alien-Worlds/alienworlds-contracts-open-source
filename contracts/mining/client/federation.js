...
const lock = async (token_account, account, quantity, eos_api) => {
    const actions = [];
...
const setLockTime = async (token_account, account, lock_time_seconds, token_symbol, eos_api) => {
    const actions = [];
    ...
    actions.push({
        ...
        data: {
            account,
            unstake_time: lock_time_seconds,
            token_symbol
        }
    });
...
const addLandData = async (asset, eos_rpc) => {
    ...
    // console.log(template_data_m)
    // console.log(template_data_i)
...
module.exports = { stake, unstake, refund, getMap, getBalance, getStaked, getUnstakes, subscribe, getAssets, getAssetById, getAssetsOld, agreeTerms, agreedTermsVersion, lock, setLockTime }
